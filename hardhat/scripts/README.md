...
EVIDENCE_CID=bafybeigdyrzt4evidence001 \
npm run disbursement:request -- --network localhost
```

Script tự tính `evidenceHash = keccak256(bytes(EVIDENCE_CID))`; không cần và không nên tự nhập hash. Nếu bỏ qua `EVIDENCE_CID`, script dùng CID mẫu. `REQUEST_ID` mặc định là request đang hoạt động, hoặc đặt rõ khi cần.
