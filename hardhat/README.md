...
Mỗi campaign nhận địa chỉ `verifier` khi được tạo. Quy trình giải ngân là: beneficiary tạo request gồm amount + CID IPFS + `evidenceHash = keccak256(bytes(CID))`, verifier của campaign duyệt, beneficiary rút đúng amount đã duyệt. Verifier không thể đổi sau khi tạo campaign; campaign mới có thể chọn ví verifier khác. Xem [`scripts/README.md`](scripts/README.md) để chạy luồng local hoàn chỉnh.
